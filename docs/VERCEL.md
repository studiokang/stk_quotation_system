...
`turbo.json`의 **`globalEnv`** 및 **`tasks.build.env`** / **`tasks.db:generate.env`** 에 나열된 변수를 Vercel **Environment Variables**에 모두 등록해야 합니다.  
`build` 태스크의 `env` 배열은 해당 값이 바뀔 때 캐시 무효화에도 쓰입니다.
